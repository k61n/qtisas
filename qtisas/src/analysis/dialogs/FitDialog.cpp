...
void FitDialog::applyChanges()
{
	...
	if (d_current_fit && QString(d_current_fit->metaObject()->className()) != "PolynomialFit" &&
		QString(d_current_fit->metaObject()->className()) != "LinearFit" && QString(d_current_fit->metaObject()->className()) != "LinearSlopeFit")
...
void FitDialog::saveUserFunction()
{
	...
	if (lst.contains(name)){
        ...
        QString fn = ApplicationWindow::getFileName(app, tr("QtiSAS") + " - " + tr("Save Fit Model As"),
                                                    app->fitModelsPath + "/" + name, filter, nullptr, true);
		if (!fn.isEmpty()){
            QFileInfo fi(fn);
...
void FitDialog::showFitPage()
{
    ...
    if (d_current_fit->type() == Fit::BuiltIn &&
		(QString(d_current_fit->metaObject()->className()) == "PolynomialFit" || QString(d_current_fit->metaObject()->className()) == "LinearFit"
		||QString(d_current_fit->metaObject()->className()) == "LinearSlopeFit")){
...
void FitDialog::accept()
{
	...
	if (!error){
		...
		if (QString(d_current_fit->metaObject()->className()) != "PolynomialFit" && QString(d_current_fit->metaObject()->className()) != "LinearFit" && QString(d_current_fit->metaObject()->className()) != "LinearSlopeFit")
