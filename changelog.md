...
# v0.13.0
* Graphs and plots feature enhanced auto-scaling and optimized export, including improved layers and SVG/PDF output
* Added configurable clipboard export options for graphs and plots (PDF, SVG, PNG, etc.)
* Fixed Columns and Rows plots for tables containing only a single point
* Fixed autoscaling for Columns and Rows plots, correctly accounting for offset and bar width
* Added transpose function for the active table
* Fittable: Data and Parameters tables now allow displaying only the necessary columns
* Jnse: Correctly imports *.dtr files and communicates with the Fittable interface
* Fixed crash on exit when a maximized widget is active
* Some 3D graph functionality is moved to qwtplo3d library
* Fixed crash when restoring a project with 3D graph
* 3D graphs are now displayed properly
* Built-in libs are now configured as targets during qtisas configuration
* For macOS there is a single universal bundle
