...
/*     Note that the DataG that define the equality constraints come */
/*     before the DataG of the inequalities. */
...
double oldmin=1e+10;
double tolmin(DataG  &x,MinInfo &Info)
{
	double fmin;
	oldmin=1e+10;
	totcal_1.itnocs =0 ;
		...
		MinInfo &info)
{
	...
	*f=info.p->funmin(Xa);
	info.p->granal(Xa,Ga);
    {
        int dimension = info.p->getDimension();
        int N=info.p->fln.n;
        double aaa=info.p->besty;
        aaa=aaa/(N-dimension);
        printf("# local: genmin    %4d[<%4d]\tchi^2\t%15.10e\t%10.5lg [<%10.5lg]\t[",info.fevals,info.iters,aaa,*f,oldmin);
