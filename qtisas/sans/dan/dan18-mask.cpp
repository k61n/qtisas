/***************************************************************************
 ...
 Description  : SANS Data Analisys Interface: mask tools
...
void dan18::createMaskFul( QString maskName )
{
    ...
    //+++
    QList<MdiSubWindow*> windows = app()->windowsList();
    foreach(MdiSubWindow *w, windows) if (w->name()==maskName && w->isA("Matrix"))
    {
        m=(Matrix *)w;
        YN=false;
        break;
...
void dan18::readMaskParaFromMaskMatrix( const QString &name )
{
    ...
    //+++
    QList<MdiSubWindow*> windows = app()->windowsList();
    foreach(MdiSubWindow *w, windows) if (w->name()==name && w->isA("Matrix"))
    {
...
void dan18::matrixList( QString selectedName)
{
    ...
    //+++
    QList<MdiSubWindow*> windows = app()->windowsList();
    foreach(MdiSubWindow *w, windows) if ( w->isA("Matrix")  ) lst<<w->name();
...
{
    QList<MdiSubWindow*> windows = app()->windowsList();
    foreach(MdiSubWindow *w, windows) if ( w->isA("Matrix") && w->name()==comboBoxMlistMask->currentText())
    {
        Matrix *m=(Matrix *)w;
