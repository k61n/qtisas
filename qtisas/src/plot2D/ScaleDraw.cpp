...
Copyright (C) by the authors:
    ...
    2009 emv <emv@5a6a7de5-fb12-0410-b871-c33778c25c60>
            ...
            if (list.length() > 1)
            {
                ...
                else
                {
                    if (d_numeric_format == SuperscriptsGER)
                        return list[0] + QString(QChar(0x00B7)) + "10<sup>" + s + "</sup>";
                    else
...
double ScaleDraw::transformValue(double value) const
{
	if (!d_formula.isEmpty()){
		...
		try{
            ...
            parser.SetExpr(d_formula.toLower().toLocal8Bit().constData());
			lbl = parser.Eval();
        }
