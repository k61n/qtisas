/***************************************************************************
 File : dan18-merge1d.cpp
 Project  : QtiSAS
 --------------------------------------------------------------------
 Copyright: (C) 2012-2021 by Vitaliy Pipich
 Email (use @ for *)  : v.pipich*gmail.com
 Description  : SANS Data Analisys Interface: ascii merging tools
...
bool dan18::addTable(const QString table, gsl_matrix* &data, int &N, int Rows, int overlap, int &firstPosition)
{
    ...
    foreach (MdiSubWindow *w, windows)
    {
        if (w->name()==table && w->isA("Table")) t=(Table *)w;
    }
    //--- cross check
...
void dan18::mergeMethodTOF(bool asciiYN)
{
    ...
    if (app()->activeWindow() && app()->activeWindow()->isA("Table")) t = (Table*)app()->activeWindow();
