...
void Graph3D::updateScalesFromMatrix(double xl, double xr, double yl, double yr, double zl, double zr)
{
	...
	if (xmin <= xStart && xmax >= xEnd && ymin <= yStart && ymax >= yEnd){
        ...
        if (rows == 0)
            rows = d_matrix->numRows();
        if (cols == 0)
            cols = d_matrix->numCols();
...
void Graph3D::setDataColorMap(LinearColorMap *colorMap)
{
    ...
    const QwtInterval range = QwtInterval(zmin, zmax);
...
Graph3D* Graph3D::restore(ApplicationWindow* app, const QStringList &lst, int fileVersion)
{
	...
	if (date.isEmpty())
        date = app->locale().toString(QDateTime::currentDateTime(), QLocale::ShortFormat);
